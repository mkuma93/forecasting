"""
Hierarchical Attention Architecture for DeepSequence with TabNet

Multi-level attention mechanism with TabNet encoders for sparse, interpretable forecasting:

1. Component-level Processing (TabNet + Dense):
   - Trend: TabNet encoder for time features
   - Seasonal: TabNet encoder for Fourier features  
   - Holiday: TabNet encoder with attention on holiday proximity
   - Regressor: TabNet encoder for lag features
   Each component uses TabNet's built-in feature selection
   
2. Feature-level Attention (Within Each Component):
   - Sparse attention on component hidden representations
   - Example: "In trend, features 3, 7, 12 matter; rest zeros"
   - Per-SKU learned via ID embedding
   
3. Component-level Ensemble (Flexible Softmax Weights):
   - Learnable weights across active components (1-4)
   - Example: "For SKU_A: trend=60%, seasonal=30%, holiday=10%"
   - Dynamically adapts to available components
   - Single component bypasses ensemble

Benefits:
- TabNet's interpretable feature selection per component
- Multi-level sparsity (TabNet + attention layers)
- Flexible ensemble (handles 1-4 components dynamically)
- SKU-specific patterns via embedding
...
└───┬────────┘    └────┬─────┘   └─────┬─────┘  └────┬────┘ │
    │                  │               │             │       │
    │ Feature Attention (Level 1 - per component)    │       │
    ▼                  ▼               ▼             ▼       │
┌───────────┐    ┌─────────┐     ┌─────────┐  ┌─────────┐  │
│Sparse Attn│    │Sparse A.│     │Sparse A.│  │Sparse A.│  │
│[batch,32] │    │[batch,32]│    │[batch,32]│  │[batch,32]│ │
└───┬───────┘    └────┬────┘     └─────┬───┘  └────┬────┘  │
    │                 │                │            │       │
...
└───┬──────┘    └────┬─────┘    └─────┬────┘  └────┬─────┘ │
    │                │                │            │       │
    │ Flexible Softmax Ensemble (Level 2) ◄───────────────┘
    │ (Dynamic: 1-4 components based on availability)
    │                │                │            │
    ▼                ▼                ▼            ▼
┌──────────┐    ┌──────────┐    ┌──────────┐  ┌──────────┐
│×w1 (SKU) │    │×w2 (SKU) │    │×w3 (SKU) │  │×w4 (SKU) │
└───┬──────┘    └────┬─────┘    └─────┬────┘  └────┬─────┘
    │                │                │            │
    └────────────────┴────────────────┴────────────┘
                     │
    ...
    ┌────────────────┴────────────────┐
    │                                  │
    ▼ (if intermittent handling)      │
┌──────────────────────────┐          │
│ Hierarchical Attention   │          │
│ Intermittent Handler     │          │
│  - Component-level attn  │          │
│  - Feature-level attn    │          │
│  - Deep Cross Network    │          │
│  → Zero Probability      │          │
└──────┬───────────────────┘          │
...
@tf.keras.utils.register_keras_serializable(package='DeepSequence')
class Mish(tf.keras.layers.Layer):
    """
    Mish activation function: f(x) = x * tanh(softplus(x))
    
    Serializable custom activation layer that can be saved and loaded properly.
...
if TFRS_AVAILABLE:
    ...
    class CrossLayer(tf.keras.layers.Layer):
        ...
        Expects input as tuple: (x, x0) where:
        - x: current layer input
        - x0: initial input (first layer's input)
        """
        ...
        def build(self, input_shape):
            # Handle both tuple input and single tensor
            if isinstance(input_shape, tuple) and len(input_shape) == 2:
                self.dim = input_shape[0][-1]  # x shape
            else:
        ...
        def call(self, inputs):
            # Handle both tuple input (x, x0) and single tensor (first layer)
...
# Helper function to get zeros tensor compatible with Keras 3
def get_zeros_like_batch(reference_tensor, units):
    """
    Create a zeros tensor with batch size from reference tensor.
    Compatible with Keras 3 KerasTensor.
    """
    return Lambda(lambda x: tf.zeros((tf.shape(x)[0], units)))(reference_tensor)
...
class ComponentBuilder:
    ...
    def get_activation_fn(self):
        """Get activation function - returns serializable activation."""
        if self.activation == 'mish':
            return Mish()  # Serializable custom layer
...
class SeasonalComponentBuilder(ComponentBuilder):
    ...
    def build(
        ...
        if not feature_indices:
            ...
            seasonal_out = get_zeros_like_batch(main_input, self.hidden_units)
            seasonal_forecast = get_zeros_like_batch(main_input, 1)
...
class HolidayComponentBuilder(ComponentBuilder):
    ...
    def build(
        ...
        if not feature_indices:
            ...
            holiday_out = get_zeros_like_batch(main_input, self.hidden_units)
            holiday_forecast = get_zeros_like_batch(main_input, 1)
...
class RegressorComponentBuilder(ComponentBuilder):
    ...
    def build(
        ...
        if not feature_indices:
            ...
            regressor_out = get_zeros_like_batch(main_input, self.hidden_units)
            regressor_forecast = get_zeros_like_batch(main_input, 1)
...
class TrendComponentBuilderSimple(ComponentBuilder):
    ...
    def build(
        ...
        # feature_dim = internal processing dimension
        # Set to match input: n_changepoints features → n_changepoints processing dim
        trend_hidden = TabNetEncoder(
            feature_dim=self.n_changepoints,  # Match input dimension
...
class SeasonalComponentBuilderTabNet(ComponentBuilder):
    ...
    def build(
        ...
        # TabNet feature_dim = internal processing dimension (NOT input dim)
        # TabNet automatically adapts to input shape in build()
        # Scale processing dim based on input size for efficiency
        n_features = len(feature_indices)
        feature_dim = (
            self.tabnet_feature_dim if self.tabnet_feature_dim
            else max(8, min(n_features * 2, 64))  # 2x input size, max 64
        )
...
class HierarchicalAttentionIntermittentHandler(ComponentBuilder):
    ...
    def build(
        ...
        if self.num_cross_layers > 0:
            x = combined_features
            x0 = combined_features  # Store initial input for cross layers
            
            # Apply cross layers
            # TFRS Cross layer expects call signature: (x0, x) or (x0) for first layer
            for i in range(self.num_cross_layers):
                if i == 0:
                    # First layer: pass single input (x0=x)
                    x = CrossLayer(name=f'cross_layer_{i+1}')(x0)
                else:
                    # Subsequent layers: pass (x0, x) as separate positional args
                    x = CrossLayer(name=f'cross_layer_{i+1}')(x0, x)
...
class DeepSequencePWLHierarchical:
    ...
    def __init__(
        ...
        activation: str = 'mish',
        data_frequency: str = 'daily',
        use_component_ensemble: bool = True  # NEW: Enable softmax-weighted ensemble
    ):
        ...
        self.activation = activation
        self.data_frequency = data_frequency
        self.use_component_ensemble = use_component_ensemble  # NEW
    ...
    def build_model(
        ...
        # Combine component forecasts (weighted ensemble or simple addition)
        # Dynamically determine available components
        available_components = []
        component_names = []
        
        if trend_feature_indices:
            available_components.append(trend_forecast)
            component_names.append('trend')
        if seasonal_feature_indices:
            available_components.append(seasonal_forecast)
            component_names.append('seasonal')
        if holiday_feature_indices:
            available_components.append(holiday_forecast)
            component_names.append('holiday')
        if regressor_feature_indices:
            available_components.append(regressor_forecast)
            component_names.append('regressor')
        
        num_components = len(available_components)
        
        if self.use_component_ensemble and num_components > 1:
            # NEW: Flexible learnable softmax-weighted combination
            # Only create weights for available components
            ...
            component_weights = Lambda(
                lambda x: tf.nn.softmax(x / 1.0, axis=-1),  # Temperature = 1.0
                name='component_weights'
            )(component_weight_logits)
            
            # Dynamically create weighted components
            weighted_components = []
            for i, (component, name) in enumerate(
                zip(available_components, component_names)
            ):
                weight = Lambda(
                ...
                weighted = Multiply(
                    name=f'{name}_forecast_weighted'
                )([component, weight])
                weighted_components.append(weighted)
            
            # Final additive combination (weighted by softmax)
            base_forecast = Add(name='base_forecast')(weighted_components)
        else:
            # Simple additive combination (equal weights or single component)
            if num_components == 1:
                base_forecast = available_components[0]
            else:
                base_forecast = Add(name='base_forecast')(available_components)
